...
####### change by HG
# p <- rep(seq(0,1,0.01),101)
# s <- sort(rep(seq(0,1,0.01),101))
# delta <- unlist(lapply(seq(0,1,0.01),Levene))
# data <- data.frame(p,s,delta)
...
# power function for modified SGS
Power <- function(N, K, data)
{
  d <- data
  d$power <- NA
  ...
  for (r in (1:nrow(data)))
  {
    ...
    for (t in (1:100))
    {
      PV <- c(PV, (length(which(abs(Ddelta(IAC[t],N,K,theta,10000))>=abs(DAF[t])))+1)/10001)
      PVfish <- c(PVfish, fisher.test(matrix(c(IAC[t],(2*N)-IAC[t],SAC[t],(2*K)-SAC[t]),nrow=2))$p)
    }
    d$power[r] <- length(which(PV<0.05))
    d$powerfish[r] <- length(which(PVfish<0.05))
...
setwd("~/Dropbox/Mac/Documents/HG/DelBay19_adult/24_power")
POW <- Power(50,50,data)
write.table(POW, "./POW50_SGS_fish_s0_0.5.csv", sep=",", quote=T, row.names=T, col.names=NA)
