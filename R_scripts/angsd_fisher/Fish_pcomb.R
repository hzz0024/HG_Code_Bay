...
option_list = list(
    make_option(c("-o", "--file1"), type="character", default=NULL,
              help="output1", metavar="character"),
    make_option(c("-p", "--file2"), type="character", default=NULL,
              help="output2", metavar="character"),
    make_option(c("-g", "--global"), type="character", default=NULL,
              help="global name", metavar="character")
);
...
fisher_method <- function(ps, dat, outall, out1, out5){
  ...
  #idx = adj
  datall = paste0(dat$V1,'\t',dat$V2,'\t', ps, '\t', adj)
...
fisher_method(ps, dat, outall=paste0(GLOBAL_NAME, 'out_all_fish.txt'), out1=paste0(GLOBAL_NAME, 'out_0.01_fish.txt'), out5=paste0(GLOBAL_NAME, 'out_0.05_fish.txt')) #write outliers to output files, no return
