...
##########  Delta_p plot ###########
####################################
setwd("~/Dropbox/Mac/Documents/Backup/Ryan_workplace/DelBay_adult/08_fish_exact/plot_zoom_in_example")
...
dat1$SNP <- id
dat1$deltaP <- delta_p
file2 = '2_35516661.txt' 
h2 = read.delim(file2, header = FALSE, sep=',')
h2 = as.list(h2)

par(mar=c(4,8,1,6))
par(mfrow=c(2,2))
manhattan(chr="chromo",bp="position",p="deltaP", snp = "SNP", subset(dat1, chromo == 2), xlim = c(35500000, 35600000), highlight1 = h2$V1, logp=FALSE, cex.axis = 1.2, ylim = c(-0.6, 0.6),
          col=c("grey50","black"),genomewideline=F, suggestiveline=F,
          ylab=expression('Surv - Ref '*Delta~italic(p)), cex.lab=1.5) 
abline(h=0, lty = 2, col = "red")
...
dat1$SNP <- id
dat1$deltaP <- delta_p
file2 = '3_43430441.txt' 
h2 = read.delim(file2, header = FALSE, sep=',')
h2 = as.list(h2)

par(mar=c(6,6,4,4))
par(mfrow=c(1,1))
manhattan(chr="chromo",bp="position",p="deltaP", snp = "SNP", subset(dat1, chromo == 3), xlim = c(43300000, 43500000), highlight1 = h2$V1, logp=FALSE, cex.axis = 1.2, ylim = c(-0.6, 0.6),
          col=c("grey50","black"),genomewideline=F, suggestiveline=F,
          ylab=expression('Surv - Ref '*Delta~italic(p)), cex.lab=1.5) 
abline(h=0, lty = 2, col = "red")
...
par(mar=c(4,8,1,6))
par(mfrow=c(2,2))
manhattan(chr="chromo",bp="position",p="deltaP", snp = "SNP", subset(dat1, chromo == 7), xlim = c(8700000, 8900000), highlight1 = h2$V1, logp=FALSE, cex.axis = 1.2, ylim = c(-0.6, 0.6),
          col=c("grey50","black"),genomewideline=F, suggestiveline=F,
          ylab=expression('Surv - Ref '*Delta~italic(p)), cex.lab=1.5) 
abline(h=0, lty = 2, col = "red")
