...
if(SAMPLE==TRUE){
  idx = seq(1100000,1700000) # sampling first 100000
  #idx = sample(seq(1,length(dat[,1])), 100000)
  ps = ps[idx,]
...
plot.boot.bars = function(DF, DF.ps, grab.size, cut, null.dist, MAIN, bar.Ycoord, bar.width, easy.cut, hard.cut){
  ...
  for (i in lgs){
    ...
    for (win in seq(1:(length(window.sides[,1])))){
      ...
      cM = median(sub$CM)
      x = append(x, cM)
      obs.count.c = c(obs.count.c, obs.count)
    }
  }
  result1 = data.frame(x, boot.ps, window.left.bounds, window.right.bounds, obs.count.c)
...
for(grab.size in seq(100, 100, 1000)){
  cut = 0.05
  ...
  results = plot.boot.bars(DF, ps, grab.size, cut, ps.boot, 'Test', .825*setYlim[1], -.75*setYlim[1], 0.1, 0.05)
  dev.off()
  
  result = results[1]
  result = data.frame(result)
  write.csv(result, paste0(grab.size, '_grab.size.csv'))
