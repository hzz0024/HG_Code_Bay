...
if(SAMPLE==TRUE){
  ...
  idx = sample(seq(1,length(dat[,1])), 80000)
  ps = ps[idx,]
...
get.bootstrap = function(DF, id, grab.size, cut){
  ...
  write.table(results, paste('nullDist', id, grab.size, cut, sep = "_"), row.names = F, quote = F)
  return(results)
...
#lefts = sapply(bars$lefts, reverse_pos)
#rights = sapply(bars$rights, reverse_pos)
#outputs = data.frame(lefts, rights, bars$cut)
...
bars1 = plot.boot.bars(DF, ps, grab.size, cut, ps.boot, 'Test', .825*setYlim[1], -.75*setYlim[1], 0.05, 0.001)
dev.off()
...
bars2 = plot.boot.bars(DF, ps, grab.size, cut, ps.boot, 'Test', .825*setYlim[1], -.75*setYlim[1], 0.05, 0.005)
dev.off()
