...
##########  Delta_p plot ###########
####################################
setwd("~/Documents/HG/DelBay_adult/13_env_gen_association/RDA/03_rda/plot")
source("manhattan.R")
library(ggplot2)
library(stringr)
###################### load maf values for delta_p ##########
file1 = 'salinity2_env2.txt_rda_loading.txt' # loadings for SNPs
...
lims <- mean(dat1_sort$RDA1) + c(-1, 1) * 2 * sd(dat1_sort$RDA1)     # find loadings +/-z sd from mean loading
lims[1]
lims[2]

jpeg("RDA_Manhattan.jpg", width = 16, height = 6, units = 'in', res = 300)
ggplot(dat1_sort, aes(x=position, y=RDA1))+ 
  #geom_point(aes(colour = cut(RDA1, c(-Inf, lims[1], 0.0357, Inf))),size = 0.8, show.legend = F)+
  stat_binhex(bins=50, binwidth=c(2e-1,1e-3))+
  #scale_fill_gradient(low = "grey", high = "red")
  # scale_color_manual(name = "RDA1",
  #                    values = c("(-Inf,-0.0366]" = "blue",
  #                               "(-0.0366,0.0357]" = "grey",
  #                               "(0.0357, Inf]" = "red"),
  #                    labels = c("<= 2sd", "-2sd < loadings <= 2sd", "> 2sd"))+
  theme_classic()+
  facet_grid(cols = vars(chromo), scales = "free_x", space="free_x") +
  theme(text = element_text(size=20)) +
  theme(axis.text.x = element_text(color = "grey20", size = 10)) +
  geom_hline(aes(yintercept =lims[2]), linetype="dashed", size=1, col="red", show.legend = FALSE)+
  geom_hline(aes(yintercept =lims[1]), linetype="dashed", size=1, show.legend = FALSE) +
