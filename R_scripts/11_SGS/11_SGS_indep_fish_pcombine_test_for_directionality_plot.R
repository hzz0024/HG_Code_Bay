...
write.table(df, "global_snp_list.txt", row.names=F, col.names = F, quote=F, sep="\t")
# this is the original delta_p results
...
dat2 <- dat1[which(dat1$adj< 0.05),]
dat2$id <- paste0(dat2$chromo,'_',dat2$position)
dat2 = dat2[with(dat2, order(chromo, position)),]
df1 <- data.frame(dat2$chromo, dat2$position)
write.table(df1, "target_snp_list.txt", row.names=F, col.names = F, quote=F, sep="\t")
