...
for(i in idxs){
  ...
  cur_pos = dat_ref$position[i] - dat_ref$position[i]%%200
  theta_key = paste0(dat_ref$chromo[i],' ',cur_pos)
  ...
  key = paste0(ref_n, ' ', ref_k, ' ', ch_n, ' ', theta)
  ...
  #ight_tail = quantile(delta_ps, .999)
  #left_tail = quantile(delta_ps, .001)
  
  #df = data.frame(p1=res$p1, p2=res$p2, delta_p=res$delta_p)
  #write.csv(df,paste0('storeP/',dat_ref$chromo[i],'_',dat_ref$position[i],'.csv'))
