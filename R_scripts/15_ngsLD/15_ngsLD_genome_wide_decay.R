...
####################################
setwd("~/Dropbox/Mac/Documents/HG/DelBay_all_angsd_final/15_ngsLD/no_shared/genome_wide_LD_decay")
...
HC <- format("HC.500.ngsld.output", 500)
ARN <- format("ARN.500.ngsld.output", 500)
COH <- format("COH.500.ngsld.output", 500)
SR <- format("SR.500.ngsld.output", 500)
NB <- format("ARN.500.ngsld.output", 500)
CHR19 <- format("CHR19.500.ngsld.output", 500)
REF19 <- format("REF19.500.ngsld.output", 500)
CHR20 <- format("CHR20.500.ngsld.output", 500)
REF20 <- format("REF20.500.ngsld.output", 500)

out = list(HC = HC, ARN = ARN, COH=COH, SR=SR, NB=NB, CHR19=CHR19, REF19=REF19, CHR20=CHR20, REF20=REF20)
save(out, file = "ngsLD.RData")
...
pop <- names(out)
meta <- read.csv('meta_pops_V1.csv', stringsAsFactors=T) # added by @HG
...
for (i in 2:10)
{
  plot(1,1,xlim=c(0,500),ylim=c(0,1),xlab="distance (kbp)", ylab="LD (r^2)",type="n")
  tmp <- out[site==levels(site)[i]] # it is odd that levels(site)[1] returns nothing so I started with 2
  ...
  for(j in 1:length(tmp))
  {
    ...
    if(length(modelC)>1)
    {
      ...
      stats <- rbind(stats,data.frame(pop=names(tmp)[j],
    ...
    else {stats <- rbind(stats,data.frame(pop=names(tmp)[j],
                                          #chr=substr(names(tmp)[j],5,9),
                                          halfdecaydist=NA,
                                          halfdecaydist=NA,
