...
write_table <- function(res_name, output_fdr1, output_fdr5){
  ...
  print('Done...')
  return(length(fdr[fdr<0.01]))
}
write_table("fish_CH_REF_pvalue.txt",  "./results/Fish_CH_REF_fdr1.txt" , "./results/Fish_CH_REF_fdr5.txt")
write_table("fish_HC_NB_pvalue.txt",  "./results/Fish_HC_NB_fdr1.txt" , "./results/Fish_HC_NB_fdr5.txt")
write_table("fish_SR_REF_pvalue.txt",  "./results/Fish_SR_REF_fdr1.txt" , "./results/Fish_SR_REF_fdr5.txt")

##################
# see shared snps
setwd("~/Documents/Ryan_workplace/DelBay/Selection_fish_1/ind_ps/results/")

make_id <- function(dt_name){
  dat <- read.delim(dt_name, header = TRUE, sep=' ')
  dat_ID = paste0(dat$chr,'_',dat$pos)
  return(dat_ID)
}

file1 = "Fish_CH_REF_fdr1.txt"
file2 = "Fish_HC_NB_fdr1.txt"
file3 = "Fish_SR_REF_fdr1.txt"
...
e <- intersect(a, c)
f <- intersect(b, c)
message('First file \"',file1,'\": ',length(a),', second file \"',file2, '\": ',length(b),'. Intersection: ',length(d))
message('First file \"',file1,'\": ',length(a),', second file \"',file3, '\": ',length(c),'. Intersection: ',length(e))
message('First file \"',file2,'\": ',length(b),', second file \"',file3, '\": ',length(c),'. Intersection: ',length(f))
