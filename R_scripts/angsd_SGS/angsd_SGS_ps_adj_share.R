...
ps_adj <- function(pname, outname1, outname2) {
  ...
  adj=dat$adj
  # output a table for SNPs with p < 0.05
  outlier1 <- as.data.frame(cbind(dat$V1[ps<0.01], dat$V2[ps<0.01], dat$V3[ps<0.01], dat$V4[ps<0.01], dat$V5[ps<0.01], dat$V6[ps<0.01], dat$adj[ps<0.01], dat$id[ps<0.01]))
  colnames(outlier1) <- c("chr","pos","p1","p2","deltap","p_value","fdr", "id")
  outlier1 <- outlier1[with(outlier1, order(chr, pos)),]
  write.table(outlier1,outname1, row.names = FALSE, sep="\t", quote = FALSE)
  # output a table for SNPs with FDR < 0.05
...
ps_adj("ps_Del19_challenge.txt", "Del19_ps_outlier.list", "Del19_FDR_outlier.list")
ps_adj("ps_Del20_challenge.txt", "Del20_ps_outlier.list", "Del20_FDR_outlier.list")
ps_adj("ps_HC_SR_challenge.txt", "SR_HC_ps_outlier.list", "SR_HC_FDR_outlier.list")
ps_adj("ps_HC_NB_challenge.txt", "NB_HC_ps_outlier.list", "NB_HC_FDR_outlier.list")
...
pname = "SR_HC_ps_outlier.list" # 2-side p-value
