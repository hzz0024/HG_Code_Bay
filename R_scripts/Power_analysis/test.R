...
s <- sort(rep(seq(0.0,0.5,0.005),101))
delta <- unlist(lapply(seq(0.0,0.5,0.005),Levene))
data <- data.frame(p,s,delta)
d <- ggplot(data, aes(p, s, z = delta))
d + stat_summary2d() + scale_fill_gradient2(name = "Delta p", high = "red", mid = "grey",low = "royalblue",midpoint = 0.05,guide = "colourbar")
...
write.table(POW100, "./POW100.csv", sep=",", quote=T, row.names=T, col.names=NA)
...
# detailed plots #
D <- ggplot(POW100, aes(p, s, z = powerz))
D + stat_summary2d(bins=101) + theme_bw() + scale_fill_gradient2(name="Power Z",high="red",mid="grey",low="royalblue",midpoint=40,guide="colourbar")
E <- ggplot(POW100, aes(p, s, z = powerfish))
E + stat_summary2d(bins=101) + theme_bw() + scale_fill_gradient2(name="Power Fisher",high="red",mid="grey",low="royalblue",midpoint=40,guide="colourbar")
G <- ggplot(POW100, aes(p, s, z = (powerz-powerfish)))
G + stat_summary2d(bins=101) + theme_bw() + scale_fill_gradient2(name="power difference",high="red",mid="grey",low="royalblue",midpoint=10,guide="colourbar")

POW50 <- Power(50,50,data)
write.table(POW50, "./POW50.csv", sep=",", quote=T, row.names=T, col.names=NA)
...
plot100 <- D1 + stat_summary2d(bins=101) + theme_bw() + scale_fill_gradient2(name="Power Z",high="red",mid="grey",low="royalblue",midpoint=40,guide="colourbar")

file2 = "POW50.csv"
POW50 <- read.delim(file2, header = TRUE, sep=',')
D2 <- ggplot(POW50, aes(p, s, z = powerz))
plot50 <- D2 + stat_summary2d(bins=101) + theme_bw() + scale_fill_gradient2(name="Power Z",high="red",mid="grey",low="royalblue",midpoint=40,guide="colourbar")

grid.arrange( plot100, plot50, nrow = 1)
