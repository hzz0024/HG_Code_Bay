...
name <- "All_maf0.05_minmapq30_minq20_pctind09_CV30_masked.ibsMat"
m <- as.matrix(read.table(name))
mds <- cmdscale(as.dist(m))
anno <- read.table('approach_2/All_92_info.txt', sep="\t", header=T)
...
idx_in_range = mds$V1> -.3 & mds$V2 > -0.2
print(idx_in_range)
...
ggplot() + geom_point(data=mds, aes_string(x='V1', y='V2', color="population"))+ 
  ...
  scale_colour_manual(values=cbPalette, breaks=c("CL", "CLP", "CS19", "CS", "HC", "HC_VA","HI","SL","SM"))+
