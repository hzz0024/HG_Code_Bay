...
setwd("~/Dropbox/Mac/Documents/HG/Domestication/Manuscript/Figures/Figure_2")
...
manhattan_plot_bottom_Zfst <- function(pname, outlier_color, x_label){
  ...
  ggplot(plot_dat, aes(x=BPcum, y=zfst)) +
    # Show all points
    # Show all points
    geom_point(aes(color=as.factor(CHR)), alpha=0.5, size=0.2) +
    scale_color_manual(values = rep(c("grey90", "grey50"), 22 )) +
    ...
    geom_point(data=subset(plot_dat, is_highlight=="yes"), color=outlier_color, alpha=0.3, size=0.3) +
    facet_grid(cols = vars(CHR), scales = "free_x", space="free_x") +
    ylab(as.expression(bquote(atop(.(x_label), ZF[italic(ST)]))))+
    xlab("Chromosome")+
    #ylim(-0.5, 0.5)+
    theme_bw() +
    theme( 
...
# 546550745
# 650678864
data_vline <- read.delim("pcadapt_common_outlier_coordinates.txt", header = TRUE, sep='\t')
data_vline$Color <- as.character(data_vline$Color)
p1 <- manhattan_plot_pcadapt(pname="pop_n_477_qs_pcadapt.txt", outlier_color = "red", x_label = "pcadapt")+
...
# 546549235
# 650677354
data_vline <- read.delim("outflank_common_outlier_coordinates.txt", header = TRUE, sep='\t')
p2 <- manhattan_plot_up(pname="pop_n_477_qs_outflank.txt", outlier_color = "red", x_label = "OutFLANK")+
  geom_vline(data = data_vline, mapping=aes(xintercept = BPcum), alpha=0.2, color=data_vline$Color)
...
p <- p1+p2+p3 + plot_layout(design=layout, guides='collect') +
