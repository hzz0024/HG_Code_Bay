...
setwd("~/Dropbox/Mac/Documents/HG/Domestication/")

pops <- read.table("./00_pop_info/Site_info_for_map.txt", header = TRUE)
...
p_noinbred <- c( "#0A2C86", "#325A98",  "#1D92BD", "#3DB9C1", "#C4E9B3", "#7BD2BF", "#ECF6B9", "#EEE8AA", 
          #  MEH2         NEH1       NEH2       UMFS       DBX1       DBX2      DBX3       UNC1        UNC2              
          "#FF9117" , "#F36616", "#D83B1C", "#FEB22B", "#F9476B", "#FC709F","#E376B7", "#CF7FBC",  "#A36DC1")
full_c <- p_noinbred
p1 <- p_noinbred
...
ggplot()+
  ...
  geom_point(data=pop_coordinates, aes(x=Long,y=Lat, fill=POP), size=8,color="grey", shape=21,alpha=0.9)+
  ...
  ylab("Latitude") +
  #coord_equal() +
  coord_map(xlim = c(-87.5, -67.5),  ylim = c(24.25, 47.25), projection = "mercator")+
  theme_bw() +
