...
outflank_naive <- function(sub_name, k, ylim){
  ...
  ggplot(out_ini$results, aes(x=pvaluesRightTail)) + geom_histogram(color="black", fill="white", bins = length(seq(0,0.35, by=0.001)))+
    theme_classic() +
    theme( 
      legend.position="none",
      panel.grid.major.x = element_blank(),
  ...
  nrow(out_ini$results)
  cnt <- sum(out_ini$results$qvalues<0.05, na.rm=TRUE)
  print(paste0("number of outliers from naive practise in ", sub_name, " is: ", cnt))
  ...
  CHR = sapply(strsplit(out_ini$results$LocusName, "_"), "[[", 1)
  POS = sapply(strsplit(out_ini$results$LocusName, "_"), "[[", 2)
  daf = data.frame(CHR=CHR, POS=POS, SNP=paste0(CHR,"_",POS), Ps=out_ini$results$pvalues, qvalue=out_ini$results$qvalues)
  ...
  write.table(outlier, file = paste0(sub_name, "_outflank_naive_q05_n_", dim(outlier)[1], ".bed"), sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
  ...
  don <- daf %>% 
    ...
    labs(caption=paste0(dim(don[-log10(don$qvalue) > ylim,])[1], " out of ", dim(outlier)[1], " outliers not shown due to Y-axis setting < 15"))
    ggsave(paste0("figs/", sub_name, ".jpg"), outlier_plot, width = 9, height = 7)
}
  
outflank_naive("genetyped_data_n_509_maf05_maxmiss095_popmiss095_hwe", 2, 2)
outflank_naive("no_DBX1_UNC_MEH_MEW_MEH_n_340", 2, 2)
...
outflank_BP("genetyped_data_n_509_maf05_maxmiss095_popmiss095_hwe", 2, 3)
outflank_BP("no_DBX1_UNC_MEH_MEW_MEH_n_342", 2, 2)
outflank_BP("pop_n_477", 2, 2)
outflank_BP("pop_n_282", 2, 2)
outflank_BP("pop_n_104", 2, 2)
############################################################
