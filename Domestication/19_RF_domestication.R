...
# Import the data set, which comprises 509 individuals genotyped at 233 biallelic loci, where 0=homozygote 1, 1=heterozygote, 2=homozygote 2
...
class_data <- read.csv("genetyped_data_n_509_maf05_maxmiss095_popmiss095_hwe_outlier_n_233.csv",row.names=1, header = TRUE) #This is the simulated data from Table S8
...
class_data_corrected[,3:235] <- NA  # Keep columns 1-2 with population ID and phenotype, but then replace with NA's over which you can write the residuals.
...
class_data_corrected[,235]-LM_SNP_i$residuals  #Should all be zero if correct
...
sample_cnt <- as.integer(length(which(class_data$Class==1))*(2/3)) # 199 susceptible individuals total
length(which(class_data$Class==0)) # 203 resistant individuals total
...
for (i in seq(from = 1, to = 100 , by = 1)){  # values of ntree
  print(i)
  for (j in c(as.integer(sqrt(p)), as.integer(2*sqrt(p)), as.integer(0.1*p), as.integer(0.3*p), as.integer(p/3), p)){    #values of mtry based on 1000 total loci
    rf_ij <- randomForest(x = class_data_corrected[,3:235], y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, ntree=i, mtry=j, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# plot(results_optimization$ntree[results_optimization$mtry == 8],results_optimization$OOB_ER[results_optimization$mtry == 8], type="l", col="black", xlab="ntree",ylab="OOB-ER",ylim=c(0,1))
# lines(results_optimization$ntree[results_optimization$mtry == 16],results_optimization$OOB_ER[results_optimization$mtry == 16], col="blue")
# lines(results_optimization$ntree[results_optimization$mtry == 32],results_optimization$OOB_ER[results_optimization$mtry == 32], col="blue")
...
ggplot(results_optimization, aes(x=ntree, y=OOB_ER))+
  geom_point(shape=1) +
  facet_grid(mtry~.)+
  ...
  labs(caption = "15, 23, 30, 69, 77, 233 represent sqrt(p), , 0.1(p), 2*sqrt(p), 0.2(p), p/3, and p, where p is the number of loci")

graph2ppt(file="1_OOB_ER_vs_ntree", width=12, height=10) 
...
rf_all_1 = randomForest(x = class_data_corrected[,3:235], y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=233, ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_all_1,file="rf_all_1.Rdata")

rf_all_2 = randomForest(x = class_data_corrected[,3:235], y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=233, ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
cor(importance_rf_all_1,importance_rf_all_2) # A correlation of 0.98 for locus importance values between forests is extremely good, so we'll use ntree trees for the remaining forests

rf_all_3 = randomForest(x = class_data_corrected[,3:235], y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=233, ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
write.csv(importance_rf_all,file="rf_importance_values_outlier_n_233.csv",row.names=FALSE)
read.csv("rf_importance_values_outlier_n_233.csv")
...
rf_2perc_1 = randomForest(x = genotypes_2perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_2perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_2perc_1,file="rf_2perc_1.Rdata")

rf_2perc_2 = randomForest(x = genotypes_2perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_2perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_2perc_2,file="rf_2perc_2.Rdata")

rf_2perc_3 = randomForest(x = genotypes_2perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_2perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# Now conduct RF on this subset
rf_3perc_1 = randomForest(x = genotypes_3perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_3perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_3perc_1,file="rf_3perc_1.Rdata")

rf_3perc_2 = randomForest(x = genotypes_3perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_3perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_3perc_2,file="rf_3perc_2.Rdata")

rf_3perc_3 = randomForest(x = genotypes_3perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_3perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# Now conduct RF on this subset
rf_4perc_1 = randomForest(x = genotypes_4perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_4perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_4perc_1,file="rf_4perc_1.Rdata")

rf_4perc_2 = randomForest(x = genotypes_4perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_4perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_4perc_2,file="rf_4perc_2.Rdata")

rf_4perc_3 = randomForest(x = genotypes_4perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_4perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# Now conduct RF on this subset
rf_5perc_1 = randomForest(x = genotypes_5perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_5perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_5perc_1,file="rf_5perc_1.Rdata")

rf_5perc_2 = randomForest(x = genotypes_5perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_5perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_5perc_2,file="rf_5perc_2.Rdata")

rf_5perc_3 = randomForest(x = genotypes_5perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_5perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# Now conduct RF on this subset
rf_10perc_1 = randomForest(x = genotypes_10perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_10perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_10perc_1,file="rf_10perc_1.Rdata")

rf_10perc_2 = randomForest(x = genotypes_10perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_10perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_10perc_2,file="rf_10perc_2.Rdata")

rf_10perc_3 = randomForest(x = genotypes_10perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_10perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# Now conduct RF on this subset
rf_20perc_1 = randomForest(x = genotypes_20perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_20perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_20perc_1,file="rf_20perc_1.Rdata")

rf_20perc_2 = randomForest(x = genotypes_20perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_20perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_20perc_2,file="rf_20perc_2.Rdata")

rf_20perc_3 = randomForest(x = genotypes_20perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_20perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
# Now conduct RF on this subset
rf_30perc_1 = randomForest(x = genotypes_30perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_30perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_30perc_1,file="rf_30perc_1.Rdata")

rf_30perc_2 = randomForest(x = genotypes_30perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_30perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_30perc_2,file="rf_30perc_2.Rdata")

rf_30perc_3 = randomForest(x = genotypes_30perc, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_best_30perc_unique), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
All_initial_err.rate$Number_loci<-c(233,length(names_best_2perc_unique),length(names_best_3perc_unique),length(names_best_4perc_unique),length(names_best_5perc_unique),length(names_best_10perc_unique),length(names_best_20perc_unique),length(names_best_30perc_unique))
...
rf_purging_1 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_purging), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_purging_1,file="rf_purging_1.Rdata")
rf_purging_2 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_purging), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_purging_2,file="rf_purging_2.Rdata")
rf_purging_3 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_purging), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
for (i in 1:(length(names_purging)-2)){  # RF cannot be conducted with 1 locus, which is why the loop is from 1:length(names_purging)-2
  ...
  rf_purging_1 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_keep), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
  rf_purging_2 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_keep), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
  rf_purging_3 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_keep), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
...
#################### Backward purging across all loci #########################
genotypes_purging <- class_data_corrected[,3:235]
names_purging <- colnames(genotypes_purging)[3:235]
rf_purging_1 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_purging), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_purging_1,file="rf_purging_1.Rdata")
rf_purging_2 = randomForest(x=genotypes_purging, y = as.factor(class_data_corrected$Class), importance=TRUE ,proximity=TRUE, mtry=length(names_purging), ntree=ntree, strata=as.factor(class_data_corrected$Class), sampsize=sample_size)
#save(rf_purging_2,file="rf_purging_2.Rdata")
...
for (i in 1:(length(names_purging)-2)){  # RF cannot be conducted with 1 locus, which is why the loop is from 1:length(names_purging)-2
  ...
  if (length(dont_keep)==1) {
    ...
    table_keep<-all_imp[-sample(dont_keep,1)]
  }
  names_keep<-rownames(table_keep)
  names_all_iterations[[length(names_purging)-i]]<-names_keep
  genotypes_purging<-class_data_corrected[,colnames(class_data_corrected) %in% names_keep]
