...
out = list()
chr <- seq(0,9)
pop <- c("MEW1", "MEW2", "LIW1", "LIW2", "DBW1", "DBW2", "NCW1", "NCW2", "UMFS", "MEH2", "NEH1", "NEH2", "DBX1", "DBX2", "DBX3", "UNC1", "UNC2")
for (c in chr) {
  for (p in pop){
...
#cbPalette <- c( "#1BA3C6", "#2CB5C0",  "#30BCAD", "#21B087", "#33A65C", "#57A337", "#A2B627", "#F8B620", "#F8B620", "#F89217","#F06719", "#E03426",  "#F64971", "#FC719E", "#EB73B3", "#CE69BE", "#A26DC2")
...
cbPalette <- c( "#0A2C86", "#849cc1",  "#1D92BD", "#8ad5d9", "#93c47d", "#bedbb1", "#a9a9a9", "#dddddd", 
                   ...
                   "#f9476b", "#fb90a6","#fddae1", "#cf7fbc",  "#e2b2d6", "#fec155", "#e1bb94", "#fbd0a5", "#f7dd8c")
#########         Dom        wild      
...
stats$pop2 <- factor(stats$pop2, levels = c("MEW1", "MEW2", "LIW1", "LIW2", "DBW1", "DBW2", "NCW1", "NCW2", "DBX1", "DBX2", "DBX3",  "UNC1", "UNC2", "UMFS", "NEH1", "NEH2", "MEH2"))
...
gg <- ggplot(stats, aes(x=pop2, y = halfdecaydist, fill=natnon)) +
  ...
  ylim(0, max(stats$halfdecaydist)) +
  scale_y_continuous(breaks = seq(0, 50000, by = 10000))+
  xlab("") +
  ylab("Half-decay distance (bp)") +
