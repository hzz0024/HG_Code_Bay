...
var_miss <- read_delim("genetyped_data_n_514_maf01_maxmiss095_popmiss09_hwe.lmiss", delim = "\t",
...
###### ind missingness #########
################################
ind_miss  <- read_delim("genetyped_data_n_514_maf01_maxmiss095_popmiss09_hwe.imiss", delim = "\t",
                        col_names = c("ind", "ndata", "nfiltered", "nmiss", "fmiss"), skip = 1)
b <- ggplot(ind_miss, aes(fmiss)) + geom_histogram(fill = "dodgerblue1", colour = "black", alpha = 0.3)
bb<-b + theme_light() + ylab("Sample count") + xlab("proportion of missing data per individual") 
##############################################
###### allele frequency distribution #########
##############################################
var_freq <- read_delim("genetyped_data_n_514_maf01_maxmiss095_popmiss09_hwe.frq", delim = "\t",
