...
# load vcf file
vcf_file = "genetyped_data_n_509_maf05_maxmiss095_popmiss095_hwe_neutral_pruned_10K.recode.vcf"
...
library(dplyr)
plotdat_group  <- group_by(plotdat, Population) # group data by party *and* congress
mean_sd <- summarize(plotdat_group,
          mean = mean(relat),
          sd = sd(relat))
write.table(mean_sd, file = "relatedness_10K_mean_sd.txt", sep = "\t", quote = FALSE,
