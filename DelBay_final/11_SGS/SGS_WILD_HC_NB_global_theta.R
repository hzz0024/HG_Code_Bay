...
interval = floor((total_n + num_split)/num_split)
idxs = seq(interval*IDX+1, min(interval*(IDX+1), total_n))
message(idxs[1])
...
ref = 'NB_18_all_minq20_minmq30_CV30_masked.mafs'
# uncomment three lines of code below to force the ref_n unchanged
# dat_ref <- read.delim(ref, header = TRUE, sep='\t')
# ref_n = NB_18
# ref_k = round(dat_ref$knownEM*ref_n*2)
...
chr = 'HC_18_all_minq20_minmq30_CV30_masked.mafs'

# uncomment three lines of code below to force the ch_n unchanged
# dat_ch <- read.delim(chr, header = TRUE, sep='\t')
# ch_n <- HC_18
# ch_k <- round(dat_ch$knownEM*ch_n*2)
...
for(i in idxs){
  ...
  # code below is to make the ref_n unchanged
  #ref_k = round(dat_ref$knownEM[i]*ref_n*2)
  
  #theta = theta_map[[theta_key]]
  theta = 0.03 # replace the local theta with global theta (i.e. 2019 reference theta)

  obs_delta=dat_ch$knownEM[i] - dat_ref$knownEM[i]
  delta_ps = Ddelta(X=ref_k, N=ref_n, K=ch_n, theta=theta, nreps=nreps)

  # two-sided test
  p_value <- (length(delta_ps[abs(delta_ps) >= abs (obs_delta)])+1)/(length(delta_ps)+1)
  #p_value <- (length(delta_ps[abs(delta_ps) >= abs (obs_delta)]))/(length(delta_ps))

  # quantile test for p-value
  #if(obs_delta>0){
  #  res = quantile.test(delta_ps,xstar=obs_delta, quantile=0.99, alternative="quantile.less")
  #  p_value=res$p.value
  #}else if(obs_delta<0){
  #  res = quantile.test(delta_ps,xstar=obs_delta, quantile=0.01, alternative="quantile.greater")
  #}else{
  #  message('obs_delta is 0')
  #  p_value=1.0
  #}

  #cat(dat_ref$chromo[i], file=outputfile, sep='\t')
  #cat('\t')
  #cat(dat_ref$position[i], file=outputfile, sep='\t')
  #cat('\t')
  #cat(p_value, file=outputfile, sep='\t')
  #cat('\t')
  ...
  if(p_value<0.05){
    ...
    cnt = cnt + 1
    #cat(1)
    cat(paste0(dat_ref$chromo[i], '\t', dat_ref$position[i], '\t', dat_ch$knownEM[i], '\t', dat_ref$knownEM[i], '\t', obs_delta, '\t', p_value, '\t', '1', '\n'), file=outputfile, append=TRUE)
  }else{
    #cat(0)
    cat(paste0(dat_ref$chromo[i], '\t', dat_ref$position[i], '\t', dat_ch$knownEM[i], '\t', dat_ref$knownEM[i], '\t', obs_delta, '\t', p_value, '\t', '0', '\n'), file=outputfile, append=TRUE)
  }
  #cat('\n')
  flush.console()
