...
library(gridExtra)
library(patchwork)
setwd("~/Dropbox/Mac/Documents/HG/DelBay_final/Figures/Figure_3/")
...
p1 <- df %>% 
  group_by(Year) %>% 
  ggplot(aes(x=TotalMortality, y=CD5, group=Year, color=Year)) +
  scale_color_manual(values = c("Cohort_2018" = "#83D1C4", "Cohort_2019" = "#F17950"))+
  #geom_point(size=6) +
...
cbPalette <- c("#83D1C4", "#F17950", "#78517C")
...
p2 <- ggplot(plot_df, aes(x=Pop, y=AF, shape=Year, color=Year)) +
