...
library(tidyr)
library(dplyr)
setwd("~/Dropbox/Mac/Documents/HG/DelBay_final/13_env_gen_association/02_baypass")
#load bf values
bf_allsnps1<-read.table("allsnps_env.controlled.1.output_summary_betai_reg.out", header = T)
bf_allsnps2<-read.table("allsnps_env.controlled.2.output_summary_betai_reg.out", header = T)
bf_allsnps3<-read.table("allsnps_env.controlled.3.output_summary_betai_reg.out", header = T)
...
dim(SNP_pos)
tmpdata = cbind(bf_allsnps1$Bayes_Factor, bf_allsnps2$Bayes_Factor, bf_allsnps3$Bayes_Factor)
BF_median <- apply(tmpdata, 1, median, na.rm=T)
BF_median_df <- cbind(BF_median, c(rep("SA", length(BF_median)/6), rep("CD5", length(BF_median)/6),rep("CD7", length(BF_median)/6),rep("CD9", length(BF_median)/6),rep("CD11", length(BF_median)/6),rep("MAX10", length(BF_median)/6)))
BF_median_df <- as.data.frame(BF_median_df)
colnames(BF_median_df) <- c("BF_median", "index")

plot_baypass <- function(EF) {
  EF_df <- BF_median_df[which(BF_median_df$index == EF),]
  bf_pos<-cbind(SNP_pos, EF_df)
  ...
  bf_pos_sort$BF_median <- as.numeric(bf_pos_sort$BF_median)
  
  BF_cut <- 10
  outlier <- as.data.frame(cbind(bf_pos_sort$chromo[bf_pos_sort$BF_median>BF_cut], bf_pos_sort$position[bf_pos_sort$BF_median>BF_cut], bf_pos_sort$BF_median[bf_pos_sort$BF_median>BF_cut], bf_pos_sort$index[bf_pos_sort$BF_median>BF_cut]))
  colnames(outlier) <- c("chr","pos","BF","index")
  outlier <- outlier[with(outlier, order(chr, pos)),]
  outlier$BF <- as.numeric(outlier$BF)
  outlier$pos <- as.numeric(outlier$pos)
  write.table(outlier, paste0(EF, ".baypass.FDR.outlier.txt"), col.names = FALSE, row.names = FALSE, sep="\t", quote = FALSE)
  # count how many SNPs with FDR < fdr_cut
  message(paste0("BF > ", BF_cut, ": ", length(outlier$index)))
  
  fig <- ggplot(bf_pos_sort, aes(x=position, y=BF_median))+ 
  ...
  assign(paste0(EF,".out"), fig, envir = globalenv())
}

#plot_baypass("CD5")
#elist <- c("CD5")
elist <- c("SA", "CD5", "CD7","CD9","CD11","MAX10")

for (EF in elist){
  plot_baypass(EF)
}

jpeg("./Figures/CD5_MAX10.jpg", width = 18, height = 12, units = 'in', res = 300)
wrap_plots(CD5.out, MAX10.out, nrow = 2) + plot_layout(guides = 'collect')
dev.off()
...
# bf_simu<-read.table("./Simulation/simul.controlled.env.output_summary_betai_reg.out", header=T)
# head(bf_simu)
# 
...
#dat=read.table("outlier_for_lm_plot.txt", header=T)
  ggplot(dat) + 
  mytheme+
  geom_smooth(method = lm, aes(x=index, y=af, group=id), colour="grey", size=0.2, se=FALSE) + 
