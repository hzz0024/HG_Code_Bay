#!/bin/bash
start=`date +%s`
BAMLIST=/workdir/hz269/DelBay20/sample_lists/CT_dedup_overlapclipped.list # Path to a list of merged, deduplicated, and overlap clipped bam files. Full paths should be included.
...
if [ ! -f $BASEDIR'/bam/CT_for_indel_realigner.intervals' ]; then
		java -Xmx120g -jar $GATK \
			-T RealignerTargetCreator \
			-R $REFERENCE \
			-I $BAMLIST \
			-o $BASEDIR'/bam/CT_for_indel_realigner.intervals' \
			-drf BadMate
fi
...
java -Xmx120g -jar $GATK \
	...
	-R $REFERENCE \
	-I $BAMLIST \
	-targetIntervals $BASEDIR'/bam/CT_for_indel_realigner.intervals' \
