...
for(win in c(5000, 1000, 500, 100, 50)){
  new_chrom_hash <- hash()
  for(chr in seq(10)){
    key = as.character(chr)
    new_chrom_hash[[key]] = list()
  }
  ...
  for(s in ss){
    chr_ = s[[1]]
    mid_ = as.integer(s[2])
    range = c(mid_-win/2, mid_+win/2)
    pre_ranges = chrom_hash[[chr_]]
    if(length(pre_ranges)==0){
      new_chrom_hash[[chr_]] = append(new_chrom_hash[[chr_]], list(range))
    }else{
      OVERLAP = FALSE
      for(big_range in pre_ranges){
        if( (range[1]>=big_range[1] && range[1]<=big_range[2]) || ( range[2]>=big_range[1] && range[2]<=big_range[2]) ){
          new_chrom_hash[[chr_]] = append(new_chrom_hash[[chr_]], list(range))
          OVERLAP = TRUE
          break
        }
  ...
  chrom_hash = new_chrom_hash 
  #print(unlist(new_chrom_hash[['8']]))
}

overlap_outlier = unlist(lapply(chrom_hash[['8']], mean))
overlap_outlier = paste0('8_', overlap_outlier)
jpeg("CS_NEH_overlaps.jpg", width = 16, height = 9, units = 'in', res = 300)
manhattan(chr="chr",bp="mid_pos",p="fst", subset(dat, chr == 8), highlight1 = overlap_outlier, logp=FALSE, cex.axis = 1, ylim = c(0, 1), 
          col=c("grey","black"),genomewideline=F, suggestiveline=F,
          ylab="Fst", cex.lab=1.5, main = paste0("CS vs NEH Fst overlap windows from 50, 100, 500, 1000, 5000bp"), cex.main=1.5)
dev.off()
...
for(win in c(50, 100, 500, 1000, 5000, 10000)){
  ...
  cnt = length(dat[dat$fst>thred[[1]],1])
  outlier = dat[dat$fst>thred[[1]],5]
  print(paste0("Number of outlier at 99.9% quantile is ", cnt, " (window size ", win, ")" ))
  manhattan(chr="chr",bp="mid_pos",p="fst", subset(dat, chr == 8), highlight1 = outlier, logp=FALSE, cex.axis = 1, ylim = c(0, 1), 
            col=c("grey","black"),genomewideline=F, suggestiveline=F,
...
##################################### vcftools single SNP CS NEH fst plot ##################################
##################################### vcftools single SNP CS NEH fst plot ##################################
##################################### vcftools single SNP CS NEH fst plot ##################################
...
fixed <- dat1[dat1$fst == max(dat1$fst),]
for(chr in seq(10)){
  cnt = nrow(fixed[fixed$chr == chr,])
  print(paste0("Chr ", chr, " has ", cnt, " fixed SNPs"))
}

write.table(fixed, file = "CS_NEH_fixed.csv", sep = ",", quote = FALSE,
