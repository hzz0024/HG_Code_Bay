...
hudson_fst <- function(headname, titlename){
  jpeg(paste0(headname,"zfst.hudson.jpg"), width = 16, height = 9, units = 'in', res = 300)
  par(mfrow=c(2,2))
  ...
  fileConn <- file(paste0(headname,'zfst.outlier.igv'))
  ls = c()
  chrs = c()
  sts = c()
  eds = c()
  ...
  write.table(outlier_DT, file = paste0(headname,"zfst.outlier.igv"), sep = "\t", quote = FALSE,
