...
dat <- data.frame(chr=sub_DT1$scaffold, start=sub_DT1$start, start=sub_DT1$end, mid_pos=sub_DT1$mid, SNP=common_id, D1_dxy=sub_DT1$dxy_CS_DEBY, D2_dxy=sub_DT2$dxy_CS_NEH, D3_dxy=sub_DT3$dxy_SL_OBOYS2)
...
write.table(dat, file = "1K_chr8.csv", sep = ",", quote = FALSE,
...
###################### start dxy plot ######################
jpeg("Dxy_1K.jpg", width = 16, height = 9, units = 'in', res = 300)
par(mfrow=c(3,1))
manhattan(chr="chr",bp="mid_pos",p="D1_dxy", subset(dat, chr == 8), highlight1 = h1$V1, logp=FALSE, cex.axis = 1, ylim = c(0, 0.05), #xlim = c(50000000, 70000000),
          col=c("grey","black"),genomewideline=F, suggestiveline=F,
          ylab="Dxy", xlab="", cex.lab=1.5, main = "CS-DEBY Dxy (1000 bp/window)", cex.main=1.5)
manhattan(chr="chr",bp="mid_pos",p="D2_dxy", subset(dat, chr == 8), highlight1 = h1$V1, logp=FALSE, cex.axis = 1, ylim = c(0, 0.05), #xlim = c(50000000, 70000000),
          col=c("grey","black"),genomewideline=F, suggestiveline=F,
          ylab="Dxy", xlab="", cex.lab=1.5, main = "CS-NEH Dxy (1000 bp/window)", cex.main=1.5)
manhattan(chr="chr",bp="mid_pos",p="D3_dxy", subset(dat, chr == 8),  highlight1 = h1$V1, logp=FALSE, cex.axis = 1, ylim = c(0, 0.05), #xlim = c(50000000, 70000000),
          col=c("grey","black"),genomewideline=F, suggestiveline=F,
          ylab="Dxy", cex.lab=1.5, main = "SL-OBOYS Dxy (1000 bp/window)", cex.main=1.5)
